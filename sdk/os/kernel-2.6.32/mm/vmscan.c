					...
					enum pageout_io sync_writeback)
{
	...
	if (pm_device_down && gpfb_canceled)
		break;
#endif
...
#ifdef CONFIG_PM_GPFB
		if (nr_freed < nr_taken && pm_device_down &&
			!gpfb_swapout_disable) {
				      ...
				      int pass, struct scan_control *sc)
{
	...
	for_each_populated_zone(zone) {
		...
		for_each_evictable_lru(l) {
			...
			reclaim_stat->nr_saved_scan[l] +=
						(lru_pages >> prio) + 1;
			if (reclaim_stat->nr_saved_scan[l] >= nr_pages ||
#ifdef CONFIG_PM_GPFB
				(pm_device_down && gpfb_shrink != GPFB_SHRINK_ALL) ||
#endif
				pass > 3) {
				unsigned long nr_to_scan;
