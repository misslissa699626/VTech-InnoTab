...
#define GP_DISP1_OUTPUT_MAX 2	/* max output device */
...
typedef struct gp_disp1_info_s
{
	...
	gp_size_t panelSize;
	gp_disp_output_t outputDev[GP_DISP1_OUTPUT_MAX];
...
static void disp1_panel_suspend(uint32_t outputIndex);
static void disp1_panel_resume(uint32_t outputIndex);
 ...
 */
static int32_t
disp1_find_output_nil_index(
	void
)
{
	int32_t i;

	for (i=0; i<GP_DISP1_OUTPUT_MAX; i++) {
		if (!gpDisp1Info->outputDev[i].ops) {
			return i;
		}
	}
...
)
{
	...
	for (i=0; i<GP_DISP1_OUTPUT_MAX; i++) {
		if (gpDisp1Info->outputDev[i].type == type &&
			strncmp(gpDisp1Info->outputDev[i].name, name, strlen(name)) == 0) {
			return i;
		}
	}
...
)
{
	...
	if (gpDisp1Info->outputDev[newOutputIndex].ops == NULL ||
		gpDisp1Info->outputDev[newOutputIndex].ops->init == NULL) {
	...
	/* Suspend old panel */
	if (gpDisp1Info->outputIndex != -1)	/* gpDisp1Info->outputIndex = -1 => Initial value */
		disp1_panel_suspend(gpDisp1Info->outputIndex);

	/* Switch to new panel */
	gpDisp1Info->outputIndex = newOutputIndex;

	/* Turn on new panel clock & set parameters */
	ret = (gpDisp1Info->outputDev[gpDisp1Info->outputIndex].ops->init)();
	if (ret < 0) {
		return ret;
	}

	/* Set data path to TFT1 or TV1 */
	if (gpDisp1Info->outputDev[gpDisp1Info->outputIndex].type == SP_DISP_OUTPUT_TV) {
...
{
	if (gpDisp1Info->outputDev[outputIndex].ops->suspend)
		(gpDisp1Info->outputDev[outputIndex].ops->suspend)();
...
{
	if (gpDisp1Info->outputDev[outputIndex].ops->resume)
		(gpDisp1Info->outputDev[outputIndex].ops->resume)();
...
)
{
	...
	index = disp1_find_output_nil_index();
	if (index < 0) {
		printk("[%s:%d], out of memory\n", __FUNCTION__, __LINE__);
		return -1;
	}

	gpDisp1Info->outputDevCnt ++;
	gpDisp1Info->outputDev[index].type = panelType;

	memset(gpDisp1Info->outputDev[index].name, 0, sizeof(gpDisp1Info->outputDev[index].name));
	strncpy(gpDisp1Info->outputDev[index].name, name, strlen(name));

	gpDisp1Info->outputDev[index].ops = panelOps;
...
)
{
	...
	index = disp1_find_output_index(panelType, name);
	if (index < 0) {
		printk("[%s:%d], can not find this entry\n", __FUNCTION__, __LINE__);
		return -1;
	}

	if (index < gpDisp1Info->outputIndex) {
		gpDisp1Info->outputIndex --;
	}
	else if (index == gpDisp1Info->outputIndex) {
		gpDisp1Info->outputIndex = 0;
	}

	gpDisp1Info->outputDevCnt --;
	for (i = index; i < gpDisp1Info->outputDevCnt; i++) {
		gpDisp1Info->outputDev[i] = gpDisp1Info->outputDev[i + 1];
	}
	gpDisp1Info->outputDev[gpDisp1Info->outputDevCnt].ops = NULL;
...
)
{
	...
	switch (cmd) {
		...
		case DISPIO_GET_PANEL_SIZE:
			{
				gp_size_t size;
				if (gpDisp1Info->outputDev[gpDisp1Info->outputIndex].ops->get_size) {
					(gpDisp1Info->outputDev[gpDisp1Info->outputIndex].ops->get_size)(&size);
		...
		case DISPIO_SET_OUTPUT:
			err = disp1_switch_panel((uint32_t) arg);
			break;

		case DISPIO_GET_OUTPUT:
			copy_to_user ((void __user *) arg, (const void *) &gpDisp1Info->outputIndex, sizeof(uint32_t));
			break;
		...
		case DISPIO_ENUM_OUTPUT:
			{
				...
				if (index < gpDisp1Info->outputDevCnt) {
					copy_to_user ((void __user *) &pOutputDev->type, (const void *) &gpDisp1Info->outputDev[index].type, sizeof(uint32_t));
					copy_to_user ((void __user *) &pOutputDev->name, (const void *) &gpDisp1Info->outputDev[index].name, sizeof(gpDisp1Info->outputDev[index].name));
				}
				else {
					err = -EIO;
				}
		...
		case DISPIO_SET_RESUME:
			gpDisp1Info->state = DISP_STATE_RESUME;
			disp1_panel_resume(gpDisp1Info->outputIndex);
			break;
...
)
{
	...
	gpDisp1Info->state = DISP_STATE_SUSPEND;
	gpDisp1Info->outputIndex = -1;

	/* irq request */
	retval = request_irq(IRQ_2D_ENGINE, disp1_irq, IRQF_SHARED, "PPU_IRQ", (void *) gpDisp1Info);		 
	if (retval) {
...
fail_register:
	free_irq(IRQ_2D_ENGINE, (void *)gpDisp1Info);
fail_irq:
	kfree(gpDisp1Info);
	gpDisp1Info = NULL;
fail_malloc:
