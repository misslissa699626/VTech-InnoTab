...
static unsigned gDispPriFd = 0;
static unsigned gDispOsdFd[2] = {0};
...
// After this function frame end irq will come AT LEAST once.
void enable_frame_end_irq_handling(void)
{
...
)
{
	...
	// VTech modification: Original code from Ge+ not safe
	enable_frame_end_irq_handling();
...
/**
 * \brief Display close all layers
 */
void
disp_close_all_layers(
	void
)
{
	DEBUG("[%s:%d]\n", __FUNCTION__, __LINE__);
	gpHalDispSetOsdEnable(0, 0);
	gpHalDispSetOsdEnable(1, 0);
	gpHalDispSetColorBarEnable(0);
	gpHalDispSetPriEnable(0);
	gpHalDispUpdateParameter();
}
EXPORT_SYMBOL(disp_close_all_layers);
...
)
{
	if (enable) {
		...
		gpHalDispSetPriInputInfo(HAL_DISP_INPUT_FMT_RGB, HAL_DISP_INPUT_TYPE_RGB565);
...
//sz modify
static void
disp_set_pri_bitmap_addr(
	gp_bitmap_t *pbitmap
)
{
	
	memcpy(&gpDispInfo->priBitmap, pbitmap, sizeof(gp_bitmap_t));
	//gpDispInfo->priBitmap.pData = pbitmap->pData;
...
{
	gpDispInfo->osdEnable[layerNum] = enable;
...
//sz modify
//for double buffer switch mode
void
disp_set_osd_bitmap_addr(
	uint32_t layerNum,
	gp_bitmap_t *pbitmap
)
{
	...
	memcpy(&gpDispInfo->osdBitmap[layerNum], pbitmap, sizeof(gp_bitmap_t));
	//gpDispInfo->osdBitmap[layerNum].pData =  pbitmap->pData;
...
EXPORT_SYMBOL(disp_set_osd_bitmap_addr);
/**
...
{
	gpDispInfo->osdBitmap[layerNum].pData = addr;
...
)
{
	...
	/* Set data path to TFT0 or TV0 */
	if (gpDispInfo->outputDev[gpDispInfo->outputIndex].type == SP_DISP_OUTPUT_TV) {
	...
	//gpHalDispSetOsdBurst(1, 2);        //change OSD1 burst type to INCR8 mode for OSD1 shaking issue
	gpHalDispSetOsdBurst(1, 1);        //change OSD1 burst type to INCR4 mode for OSD1 shaking issue
...
/**
 * \brief Display device set Flip type
 */
void
disp_set_flip_function(
	uint32_t value
)
{
	gpDispInfo->flipEnableValue = value;
	gpHalDispSetFlip(value);
}
EXPORT_SYMBOL(disp_set_flip_function);

/**
 * \brief Display device Primary layer set Flip type
 */
void
disp_primary_set_flip_function(
	uint32_t value
)
{
	uint32_t temp;
	
	temp = gpDispInfo->flipEnableValue;
	if((value&0x1) == 1){
	    temp = temp | (0x01);
	}else{
	    temp = temp & (0xFE);
	}
	gpDispInfo->flipEnableValue = temp;
	gpHalDispSetFlip(temp);
}
EXPORT_SYMBOL(disp_primary_set_flip_function);

/**
 * \brief Display device OSD0 set Flip type
 */
void
disp_OSD0_set_flip_function(
	uint32_t value
)
{
	uint32_t temp;
...
/**
 * \brief Display device OSD1 set Flip type
 */
void
disp_OSD1_set_flip_function(
	uint32_t value
)
{
	uint32_t temp;
...
)
{
	...
	/* Flip */
	disp_set_flip_function(gpDispInfo->flipEnableValue);
...
uint32_t gVBOccur = 0;			// Ad-hoc flag to monitor the status of TFT VB
static irqreturn_t
disp_irq(
	int32_t irq,
	void *dev_id
)
{
	...
	if (gpDispInfo->intFlag == 0) {
		//VTech modify Ge+ SZ's modification: disable Frame end interrupt
		gpHalDispSetIntDisable(HAL_DISP_INT_FRAME_END);	
		gpDispInfo->intFlag = 1;
...
struct USED_LAYER_STRUCT {
	int has_ever_used_pri;
	int has_ever_used_osd[2];
};
...
{
	struct USED_LAYER_STRUCT *prv_data = 0;
	DEBUG("[%s:%d]\n", __FUNCTION__, __LINE__);

	prv_data = (struct USED_LAYER_STRUCT*)kmalloc(sizeof(struct USED_LAYER_STRUCT), GFP_KERNEL);
	if (!prv_data) {
		printk(KERN_WARNING "Failed to allocate memory for private data!\n");
		return -ENOMEM;
	}
	
	memset(prv_data, 0, sizeof(struct USED_LAYER_STRUCT));
...
)
{
	...
	prv_data = (struct USED_LAYER_STRUCT*)file->private_data;
	if (prv_data) {
		if (((unsigned)file == gDispPriFd) && (prv_data->has_ever_used_pri)) {
			disp_set_pri_enable(0);
		}
		if (((unsigned)file == gDispOsdFd[0]) && (prv_data->has_ever_used_osd[0])) {
			disp_set_osd_enable(0, 0);
		}
		if (((unsigned)file == gDispOsdFd[1]) && (prv_data->has_ever_used_osd[1])) {
			disp_set_osd_enable(1, 0);
		}
		disp_update();
...
)
{
	...
	switch (cmd) {
		...
		case DISPIO_SET_SUSPEND:
			if (gpDispInfo->state == DISP_STATE_SUSPEND) {
				break;
			}
		...
		case DISPIO_SET_FRAMERATE:
			gpHalDispSetFrameRate(arg);
			gpHalLcdClkEnable(1);
			break;

		case DISPIO_GET_FRAMERATE:
			{
				int framerate;
				framerate = gpHalDispGetFrameRate();
				copy_to_user ((void __user *) arg, (const void *) &framerate, sizeof(uint32_t));
			}
			break;
		...
		case DISPIO_SET_PRI_ENABLE:
			...
			if (prv_data) {
				prv_data->has_ever_used_pri = 1;
			}
			if (arg) {
				gDispPriFd = (unsigned)file;
			}
			break;
		...
		case DISPIO_CHANGE_PRI_BITMAP_BUF:
			{
				...
				copy_from_user((void*) &bitmap, (const void __user *) arg, sizeof(gp_bitmap_t));
				bitmap.pData = (uint8_t*) gp_user_va_to_pa(bitmap.pData);
				disp_set_pri_bitmap_addr(&bitmap);
			}
			break;	
		//-----------------------------------	
		...
		case DISPIO_SET_OSD_ENABLE(1):
			{
				...
				if (prv_data) {
					prv_data->has_ever_used_osd[layerNum] = 1;
				}
				if (arg) {
					gDispOsdFd[layerNum] = (unsigned)file;
				}
			}
			break;
		...
		case DISPIO_CHANGE_OSD_BITMAP_BUF(0):
			{
				...
				copy_from_user((void*) &bitmap, (const void __user *) arg, sizeof(gp_bitmap_t));
				bitmap.pData = (uint8_t*) gp_user_va_to_pa(bitmap.pData);
				disp_set_osd_bitmap_addr(layerNum, &bitmap);
			}
			break;			
		...
		case DISPIO_SET_FLIP:
		                 disp_set_flip_function((uint32_t) arg);
		        break;

		case DISPIO_CLR_VB_STATUS:
				gVBOccur = 0;
				// Ensure that the interrupt will come at least once
				enable_frame_end_irq_handling();
				break;
		case  DISPIO_CHECK_VB_STATUS:
			{
				uint32_t vbOccur = gVBOccur;
				copy_to_user ((void __user *) arg, (void *)&vbOccur, sizeof(uint32_t));
			}
			break;
		case DISPIO_PRIMARY_SET_FLIP:
		                 disp_primary_set_flip_function((uint32_t) arg);
		        break;
		case DISPIO_OSD0_SET_FLIP:
		                 disp_OSD0_set_flip_function((uint32_t) arg);
		        break;		        
		case DISPIO_OSD1_SET_FLIP:
		                 disp_OSD1_set_flip_function((uint32_t) arg);
		        break;	        
		default:
			DEBUG("[%s:%d], unknow cmd\n", __FUNCTION__, __LINE__);
			break;
	}
	//sz modify Enable Frame end inturrept for each command
	//gpHalDispSetIntEnable(HAL_DISP_INT_FRAME_END);
	//-----------------
...
{
	printk("remove display device ok\n");
}
