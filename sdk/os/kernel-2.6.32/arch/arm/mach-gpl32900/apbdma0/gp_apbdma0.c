...
#define gp_apbdma0_protect()	{down(&g_apb->sem); spin_lock(&g_apb->lock);}
#define gp_apbdma0_unprotect()	{spin_unlock(&g_apb->lock); up(&g_apb->sem);}
...
/**
 * @brief   Apbdma0 clock enable/disable
 * @param   enable [in] 0:disable, 1:enable
 * @return  None
 * @see
 */
static void apbdma0_clock_enable(int enable)
{
#ifdef CONFIG_PM
	gp_enable_clock((int*)"APBDMA_C", enable);
#else
	gpHalScuClkEnable(SCU_C_PERI_APBDMA, SCU_C, enable);
#endif
}
...
int gp_apbdma0_wait(unsigned int handle, unsigned short timeout)
{
	...
	{
		/* ----- Hardware issue, apbdma must control must set 1 to 0 ----- */
		gpHalApbdmaRst(handle);
		return 0;	
	}
	else
...
/**                                                                         
 * @brief   apbdma0 device release                                              
 */                                                                         
static void gp_apbdma0_device_release(struct device *dev)                       
{                                                                           
	printk("remove apbdma0 device ok\n");                                      
}                                                                           
                                                                            
static struct platform_device gp_apbdma0_device = {                             
	.name	= "gp-apbdma0",                                                         
	.id	= 0,                                                                  
	.dev	= {                                                                 
		.release = gp_apbdma0_device_release,                                       
	},                                                                        
};                                                                          
                                                                            
static int gp_apbdma0_probe(struct platform_device *pdev){       
	return 0;                                                                 
}                                                                           
                                                                            
static int gp_apbdma0_remove(struct platform_device *pdev){  
	return 0;                                                                 
}                                                                           
                                                                            
static void gp_apbdma0_suspend_set( void ){
	//printk("[%s][%d]\n", __FUNCTION__, __LINE__);
	apbdma0_clock_enable(0);
}
EXPORT_SYMBOL(gp_apbdma0_suspend_set);

static void gp_apbdma0_resume_set( void ){
	//printk("[%s][%d]\n", __FUNCTION__, __LINE__);
	apbdma0_clock_enable(1);
	gpHalApbdmaHWRst();	
}
EXPORT_SYMBOL(gp_apbdma0_resume_set);                                                               
                                                                            
#ifdef CONFIG_PM                                                            
static int gp_apbdma0_suspend(struct platform_device *pdev, pm_message_t state){
	gp_apbdma0_suspend_set();                                              
	return 0;                                                                 
}                                                                           
                                                                            
static int gp_apbdma0_resume(struct platform_device *pdev){  
	gp_apbdma0_resume_set();                                                      
	return 0;                                                                 
}                                                                           
#else                                                                       
#define gp_apbdma0_suspend NULL                                                 
#define gp_apbdma0_resume NULL                                                  
#endif                                                                      
                                                                            
/**                                                                         
 * @brief   wdt driver define                                               
 */                                                                         
static struct platform_driver gp_apbdma0_driver = {                             
	.probe  = gp_apbdma0_probe,                                                   
	.remove = gp_apbdma0_remove,                                                  
	.suspend = gp_apbdma0_suspend,                                                
	.resume = gp_apbdma0_resume,                                                  
	.driver	= {                                                               
		.owner	= THIS_MODULE,                                                  
		.name	= "gp-apbdma0"                                                        
	}                                                                     
...
int __init gp_apbdma0_init(void)
{
	...
	platform_device_register(&gp_apbdma0_device);
	return platform_driver_register(&gp_apbdma0_driver);
out_irq:
	if(i)
	{
		/* ----- free irq function ----- */
...
static void __exit gp_apbdma0_exit(void)
{
	...
	platform_device_unregister(&gp_apbdma0_device);
	platform_driver_unregister(&gp_apbdma0_driver);
}
