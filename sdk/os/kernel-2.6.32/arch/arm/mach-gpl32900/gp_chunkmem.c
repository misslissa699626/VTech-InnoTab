...
{
	if (chunkmem) {
		return chunkmem->pbase;
	}
	return 0;
}
EXPORT_SYMBOL(vpad_debug_chunkmem_get_phyaddr_start);

unsigned int vpad_debug_chunkmem_get_phyaddr_size(void)
{
	if (chunkmem) {
		return chunkmem->size;
	}
	return 0;
}
EXPORT_SYMBOL(vpad_debug_chunkmem_get_phyaddr_size);
...
static int __init chunkmem_init(void)
{
	...
	DIAG_INFO("[%s] pbase=%08X vbase=%08X size=%08X\n",
			  chunkmem->dev.name, chunkmem->pbase, chunkmem->vbase, chunkmem->size);
	dlMalloc_Status(NULL);
