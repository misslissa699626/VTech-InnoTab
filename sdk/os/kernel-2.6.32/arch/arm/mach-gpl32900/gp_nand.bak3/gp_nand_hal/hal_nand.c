...
void nandSetDmaTiming(struct nand_dma_timing_rec *timing, bool doWrite)
{
	...
	// The following line is what has been used for quite some time with NAND flash, before NAND ROM test is conducted.
	rFM_AC_TIMING = ( rFM_AC_TIMING & 0xff00ffff) | ( wait_no << 16); // set wait_no part

	/*  Comment from Kevin's mail on 30-07-2011:

		Case EV board:
		Set the value of rFM_AC_TIMING to 0x1F2A7727 (period of RE: >= 100ns)

		Case GA2:
		Set the value of rFM_AC_TIMING to 0x1F2A3514 (period of RE: >= 62ns)

		Please be noted that GA2 also can work if set rFM_AC_TIMING to 0x1F2A7727 (period of RE: >= 100ns), but the speed of reading will be slow down.
		BTW, the period of RE signal in storio is 100ns .
	*/
	//rFM_AC_TIMING = 0x1F2A7727;
