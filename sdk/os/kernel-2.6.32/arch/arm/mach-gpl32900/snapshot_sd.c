...
extern int SdcAppReadSector(u32, u32, u16, u32);
/* Chunk functions */
...
static int app_header_read(void *buf)
{
	...
	#else
	ret = SdcAppReadSector(0, AH_START_LBA, AH_SIZE, (u32)buf);
	#endif
	gpfb_printf("%s:%d\n", __func__, __LINE__);
	flush_cache_all();
...
static int hibdrv_copy_to_sram(void *buf, u32 start_sect, u32 work_size)
{
	...
	#else
	ret = SdcAppReadSector(0, start_sect, 1, (u32)buf);
	#endif
	...
	ret = NandAppReadSector((start_sect + 1), (sect_num - 1), 
				(u32)(buf + 512), mode);
	#else
	ret = SdcAppReadSector(0, (start_sect + 1), (sect_num - 1), (u32)(buf + 512));
	#endif
	gpfb_printf("%s:%d: ret=%d\n", __func__, __LINE__, ret);
