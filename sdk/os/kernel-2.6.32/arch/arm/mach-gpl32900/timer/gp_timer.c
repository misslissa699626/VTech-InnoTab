...
static int g_tc_regSave[5] = {0};


/**
 * @brief   Timer clock enable/disable
 * @param   enable [in] 0:disable, 1:enable
 * @return  None
 * @see
 */
static void tc_clock_enable(int enable)
{
	gpHalScuClkEnable( SCU_B_PERI_TIMER0, SCU_B, enable);
}

#if 0
static int gpioSetting = 0;
//Test program for timer interrupt test.
static void tc_test_irq(int id, void* handle){
	int index = 0, value = 0;
	//printk("[%s][%d], [%x]\n", __FUNCTION__, __LINE__, id);
	gpioSetting = gpioSetting == 0?1:0;
	index = MK_GPIO_INDEX( 2, 2, 4, 0);
	//gpHalTimerEn(id, 0);
	gpHalTimerInterruptGet(id, &value);
	if( value == 1) {
		gpHalTimerInterruptSet(id, 0);
	}
	gpHalGpioSetValue(index, gpioSetting);
}
#endif
/**
...
{	
	...
	//Test program for timer interrupt test.
	//gp_tc_irq_register(handle, &tc_test_irq);
	//gpHalTimerClkSrcSet(timer->id, 2);
...
int gp_tc_set_output_mode(int handle, int mode)
{
	...
	gpHalTimerOutputModeSet(timer->id, mode);
	
	return 0;
}
EXPORT_SYMBOL(gp_tc_set_output_mode);
...
int gp_tc_get_output_mode(int handle, int *mode)
{
	...
	*mode = gpHalTimerOutputModeGet(timer->id);

	return 0;
}
EXPORT_SYMBOL(gp_tc_get_output_mode);
...
void gp_tc_set_freq_duty(int handle, UINT32 freq, UINT32 duty)
{
	...
	gp_tc_set_load(handle,65536-temp); /* up counting */
	...
	gp_tc_set_compare(handle, 65536-temp);
...
int gp_tc_request(int timer_id, char *name)
{
	...
	if(found){
		...
		timer = NULL;
		DIAG_ERROR("request timer[%d] used,try next id\n",timer_id);
	}
	return (int)timer;
}
...
int gp_tc_release(int handle)
{
	...
	for( i=0; i<TIMER_TOTAL_NUMBER; i++ ) {
		if( gpHalTimerEnGet(i) == 1 ) {
			gp_timer_occupy++;
		}
	}
	/*Close the clock if all timer aren't used.*/
	if( gp_timer_occupy == 0 ) {
		tc_clock_enable(0);
	}
...
static long gp_tc_dev_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case TC_IOCTL_GET_COMPARE:
		...
		break;			
	case TC_IOCTL_SET_OUTPUTMODE:
		gp_tc_set_output_mode((int)timer, arg);	
		break;			
	case TC_IOCTL_GET_OUTPUTMODE:
		gp_tc_get_output_mode((int)timer, &temp);
		if(copy_to_user((void __user *)arg, &temp, sizeof(int))){
			ret = -EFAULT;
		}
		break;			
...
void gp_tc_suspend_set( int devId ){
	int* ptr; 
	if( g_tc_regSave[devId] == 0 && gpHalTimerEnGet(devId) ) {
		ptr = kmalloc( TIMER_REGISTER_OFFSET, GFP_KERNEL);
		gpHalTimerRegSave( devId, ptr );
		g_tc_regSave[devId] = (int)ptr;
		if( ptr == NULL ) {
			printk("[%s][%d]Suspend Error, it allocates memory fail\n", __FUNCTION__, __LINE__);
		}
	}
	else {
		//printk("[%s][%d], TIMER[%d] PTR[%x] MODE 0?[%d] EN[%d]\n", __FUNCTION__, __LINE__, devId, 
			//   g_tc_regSave[devId], gpHalTimerOutputModeGet(devId), gpHalTimerEnGet(devId));
	}
}
EXPORT_SYMBOL(gp_tc_suspend_set);

void gp_tc_resume_set( int devId ){
	int* ptr; 
	//printk("[%s][%d]\n", __FUNCTION__, __LINE__);
	if( g_tc_regSave[devId] != 0 ) {
		ptr = (int *)g_tc_regSave[devId];
		gpHalTimerRegRestore( devId, ptr );
		kfree( ptr );
		g_tc_regSave[devId] = 0;
	}
	else {
		//printk("[%s][%d], [%d]\n", __FUNCTION__, __LINE__, devId);
	}
}
EXPORT_SYMBOL(gp_tc_resume_set);

#ifdef CONFIG_PM
static int gp_tc_suspend(struct platform_device *pdev, pm_message_t state){

	//printk("[%s][%d][%x]\n", __FUNCTION__, __LINE__, pdev->id);
	gp_tc_suspend_set( pdev->id );
	return 0;
}

static int gp_tc_resume(struct platform_device *pdev){
	//printk("[%s][%d][%x]\n", __FUNCTION__, __LINE__, pdev->id);
	gp_tc_resume_set( pdev->id );
	return 0;
}
#else
#define gp_tc_suspend NULL
#define gp_tc_resume NULL
#endif
