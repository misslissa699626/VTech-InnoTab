...
)
{
	...
	if(uErr != SUCCESS){
		diag_printf("[check_inuse_chunk] check error\n");
		return uErr;
	}
	else if (!(is_inuse(p))){
		diag_printf("[check_inuse_chunk] is_inuse\n");
		return 1;
	}
	else if (!(next_pinuse(p))){
		diag_printf("[check_inuse_chunk] next_pinuse\n");
		return 1;
	}
	else if (!(pinuse(p) || next_chunk(prev_chunk(p)) == p)){
		/* If not pinuse and not mmapped, previous chunk has OK offset */
		diag_printf("[check_inuse_chunk] pinuse\n");
		return 1;
	}
...
)
{
	...
	uErr = check_any_chunk(m, p);
	if(uErr != SUCCESS){
		diag_printf("[check_free_chunk] check error\n");
		return uErr;
	}
	else if (is_inuse(p)){
		diag_printf("[check_free_chunk] not inuse\n");
		return 1;
	}
	else if(next_pinuse(p)){
		diag_printf("[check_free_chunk] next pinuse\n");
		return 1;
	}
	if (p != m->dv && p != m->top){
		if (sz >= MIN_CHUNK_SIZE){
			if ((sz & CHUNK_ALIGN_MASK) != 0){
				diag_printf("[check_free_chunk] check align\n");
				return 1;
			}
			else if(!is_aligned(chunk2mem(p))){
				diag_printf("[check_free_chunk] not aligned\n");
				return 1;
			}
			else if(next->prev_foot != sz){
				diag_printf("[check_free_chunk] check prev foot\n");
				return 1;
			}
			else if(!pinuse(p)){
				diag_printf("[check_free_chunk] pinuse\n");
				return 1;
			}
			else if(!(next == m->top || is_inuse(next))){
				diag_printf("[check_free_chunk] next\n");
				return 1;
			}
			else if(p->fd->bk != p){
				diag_printf("[check_free_chunk] (p->fd->bk != p)\n");
				return 1;
			}
			else if(p->bk->fd != p){
				diag_printf("[check_free_chunk] (p->bk->fd != p)\n");
				return 1;
			}
		}
...
)
{
	...
	if (mem == 0){
		diag_printf("[check_malloced_chunk] null memory\n");
		return 1;
	}
	else{
		...
		if(uErr != SUCCESS){
			diag_printf("[check_malloced_chunk] check inuse error\n");
			return uErr;
		}
		if((sz & CHUNK_ALIGN_MASK) != 0){
			diag_printf("[check_malloced_chunk] check align\n");
			return 1;
		}
		else if(sz < MIN_CHUNK_SIZE){
			diag_printf("[check_malloced_chunk] min size\n");
			return 1;
		}
		else if(sz < s){
			diag_printf("[check_malloced_chunk] no than need\n");
			return 1;
		}
		else if(sz > (s + MIN_CHUNK_SIZE)){
			/* unless mmapped, size is less than MIN_CHUNK_SIZE more than request */
			diag_printf("[check_malloced_chunk] unless mmapped\n");
			return 1;
		}
	}
...
#ifdef USE_BIG_BOUNDARY
	msize = PAD_REQUEST(sizeof(malloc_state_t));
...
)
{
	...
	if (v != 0 && rsize < (UINT32)(m->dvsize - nb)){
		if (RTCHECK(ok_address(m, v))){
			...
			if(chunksize(v) != rsize + nb){
				diag_printf("[tmalloc_large] %d v:0x%08X\n", __LINE__, (UINT32)v);
				return NULL;
			}
	...
	//vicsiu: diag_printf("[tmalloc_large] %d \n",__LINE__);
	return NULL;
}
...
)
{
	...
	if ((bytes & 0x80000000) != 0){
		diag_printf("[dlMalloc] one buf size:0x%08X\n", bytes);
		return mem;
	}
	//for Linux page control
	bytes = (bytes + CHUNK_ALIGN_MASK) & ~CHUNK_ALIGN_MASK;
	if (bytes >= MAX_REQUEST){
		diag_printf("[dlMalloc] two buf size:0x%08X\n", bytes);
	...
	//else if (bytes <= MAX_SMALL_REQUEST){
		...
		if ((smallbits & 0x3U) != 0){
			...
			mem = chunk2mem(p);
			uErr = check_malloced_chunk(pgdl_GM, mem, nb, id);
			if (uErr != SUCCESS){
				diag_printf("[dlMalloc] %d \n",__LINE__);
			}
			return mem;
...
#define MAX_SL_BUF_NUM      (128)
#define MIN_SL_NCHUNK_SIZE  (0x100000)
...
)
{
	...
	if ( pMem == NULL ) {
	    diag_printf("[dlMallocEx] first dlMalloc failed!\n");
	}
	...
	if ((ok_address(pgdl_GM, pMem)) && (bytes >= MAX_SL_BUFFER_SIZE)) {
		...
		if ((ChunkSize + NextChunkSize) >= MAX_SL_CHUNK_SIZE && NextChunkSize >= MIN_SL_NCHUNK_SIZE) {
			if (!is_inuse(pNextChunk)) {
				void * pTMem;
				UINT32 i;
				...
				if (ok_address(pgdl_GM, pTMem)) {
					pMem = dlMalloc(id, bytes);
					if ( pMem == NULL ) {
					    diag_printf("[dlMallocEx] second dlMalloc failed!\n");
					}
				}
				else{
					diag_printf("[dlMallocEx] invalid address...\n");
				}
...
)
{
	...
	diag_printf("[dlFree] invalid address! \n");
	return 1;
}
...
)
{
	...
	if(uErr != SUCCESS){
		diag_printf("[internal_free_all] invalid address! \n");
		return iFreeNum;
	}
