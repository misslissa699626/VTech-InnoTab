 ...
 **************************************************************************/
int gp_ppu_fops_open(struct inode *inode, struct file *filp);
int gp_ppu_fops_release(struct inode *inode, struct file *filp);
int gp_ppu_fops_ioctl(struct inode *inode, struct file *filp, unsigned int cmd, unsigned long arg);
static unsigned int gp_ppu_fops_poll(struct file *filp, struct poll_table_struct *poll);
...
struct file_operations ppu_fops = {
	.owner = THIS_MODULE,
	.poll = gp_ppu_fops_poll,
	.open = gp_ppu_fops_open,
	.ioctl = gp_ppu_fops_ioctl,
	.release = gp_ppu_fops_release,
};
...
#else
		gpHalScuClkEnable( SCU_A_PERI_RTABT212, SCU_A, 1);
		gpHalScuClkEnable( SCU_C_PERI_2DSCALEABT, SCU_C, 1);
#endif
		gpHalScuClkEnable( SCU_A_PERI_PPU | SCU_A_PERI_PPU_REG | SCU_A_PERI_PPU_FB, SCU_A2, 1);
	}
	else{
		//Control it by PM gpHalScuClkEnable( 1<<22, SCU_C, 0);
		gpHalScuClkEnable( SCU_A_PERI_PPU | SCU_A_PERI_PPU_REG | SCU_A_PERI_PPU_FB, SCU_A2, 0);
#ifdef CONFIG_PM
		gp_enable_clock((int*)"PPU_FB", 0);
...
)
{
	...
	free_irq(VIC_PPU, NULL);
	gpHalPPUEn(0);
	gp_ppu_module_clk_en(0);
	return 0;
}
...
/**                                                                         
 * @brief   ppu device release                                              
 */                                                                         
static void gp_ppu_device_release(struct device *dev)                       
{                                                                           
	DIAG_INFO("remove ppu device ok\n");                                      
}                                                                           
...
//EXPORT_SYMBOL(gp_ppu_suspend_set);
...
//EXPORT_SYMBOL(gp_ppu_resume_set);                                                               
                                                                            
#ifdef CONFIG_PM                                                            
static int gp_ppu_suspend(struct platform_device *pdev, pm_message_t state){
	if( ppu_init_state ) {
		gp_ppu_suspend_set();                                              
	}
	return 0;                                                                 
...
static int gp_ppu_resume(struct platform_device *pdev){  
	if( ppu_init_state ) {
		gp_ppu_resume_set();  
	}
	return 0;                                                                 
...
#else                                                                       
#define gp_ppu_suspend NULL                                                 
#define gp_ppu_resume NULL                                                  
#endif                                                                      
...
)
{
	...
	return platform_driver_register(&gp_ppu_driver);
