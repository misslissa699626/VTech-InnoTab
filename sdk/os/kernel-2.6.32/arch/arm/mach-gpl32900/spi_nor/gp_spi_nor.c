...
static void gp_spi_nor_setup_device(struct gp_spi_nor_dev *dev)
{
	...
	gp_spi_set_freq((int)dev->gd->private_data, 18000000);      //set freq
	snprintf (dev->gd->disk_name, 32, "spi_nor");
	set_capacity(dev->gd, 8192*2 ); /* 8MB */
	add_disk(dev->gd);
	return;
}
...
static int __init gp_spi_nor_init(void)
{
	...
	timerID = NULL;
	for (i=0;i<5;i++) {
		timerID = (struct gp_tc_s *)(gp_tc_request(i,"spi_nor"));
		if(timerID != NULL){
		    break;
		}
	}
	...
	/* setup timer */
	gp_tc_set_prescale((int)timerID,temp);
	gp_tc_set_operation_mode((int)timerID,1); /* period timer mode */
	gp_tc_set_count_mode((int)timerID,1);
	gp_tc_enable_int((int)timerID,1);
...
static void __exit gp_spi_nor_exit(void)
{
	...
	if (dev->gd) 
	{
		...
		gp_spi_release((int)dev->gd->private_data);
		put_disk(dev->gd);
	}		
...
static int gp_spi_nor_check_status(void)
{
	...
	/* start timer */
	gp_tc_set_load((int)timerID,65536-SPI_NOR_READ_STATUS_TIMEOUT-1);
	gp_tc_enable((int)timerID);
	
	while((gp_spi_nor_read_status() & 0x01) != 0) 
  	{
		/*printk (KERN_NOTICE "erase before SPI_Flash_ReadStatus not ready!\n");*/
		gp_tc_get_int_state((int)timerID,&iflag);
		if (iflag & 0x1) {
			printk (KERN_NOTICE "check status:Timer...time-out!\n");
			gp_tc_set_int_state((int)timerID,0);
			gp_tc_disable((int)timerID);
			return -ETIME;
		}
  	}
  	/* end timer */
