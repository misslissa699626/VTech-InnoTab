...
static int dsp_ioctl_frequency_set(struct file *filp, int frequency)
{
	...
	if ((filp->f_flags & O_ACCMODE) == O_WRONLY) {
		ret = audio2_FREQ_set(frequency, 1);
	} else if ((filp->f_flags & O_ACCMODE) == O_RDONLY) {
		ret = audio2_FREQ_set(frequency, 0);
	}
	return ret;
}
			     ...
			     struct file *filp)
{
	...
	audio2_FREQ_set(16000, 0);
	audio2_MIC_ctrl(1);

#ifdef AGCEN
	I2SRx_init_mic_agc();
#else /*  */
	audio2_MIC_volset(0x02);
#endif /*  */
...
static int dsp_resume(struct platform_device *pdev)
{
	...
	audio2_resume();
	audio2_Power_ctrl(1);
	audio2_WAVE_volset(0x10, 0x10);	/*default volume to about 50% */
	return 0;
}
