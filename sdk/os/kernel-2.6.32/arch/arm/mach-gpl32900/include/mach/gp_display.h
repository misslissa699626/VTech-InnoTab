...
#define DISPIO_CLR_VB_STATUS	_IOW(DISPIO_TYPE, 0x14, uint32_t)		// Ad-hoc commands to monitor the status of TFT VB
#define DISPIO_CHECK_VB_STATUS	_IOR(DISPIO_TYPE, 0x15, uint32_t *)		// Ad-hoc commands to monitor the status of TFT VB
...
#define DISPIO_PRIMARY_SET_FLIP     _IO(DISPIO_TYPE, 0x8a)
...
extern void disp_set_flip_function(uint32_t value);
extern void disp_close_all_layers(void);
