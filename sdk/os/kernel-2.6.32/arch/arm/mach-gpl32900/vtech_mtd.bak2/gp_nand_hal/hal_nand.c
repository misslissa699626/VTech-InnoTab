...
SINT32 nf0_init_intr(void) 
{
	...
	return request_irq(IRQ_NAND0, nf0_interrupt_isr, IRQF_DISABLED, NF0_DEVICE_NAME, NULL);
}
...
//	UINT32 i;
	...
	if(ret!=0)	// zurong add
	{
		...
		gHalNandDebugCnt ++;
		printk( "%s() - %d: Uncatched error: %d\n", __PRETTY_FUNCTION__, __LINE__ , gHalNandDebugCnt);
...
//    NFCMD_STATUS ret;
  	...
  	if(gPhysical_NandInfo->wPageSize > 512)   
  	{   /*Large Page*/
		if ( Nand_Select>= 0 && Nand_Select < (sizeof(gEraseCycle)/sizeof(gEraseCycle[0])) && gEraseCycle[Nand_Select] != 0 )
		{
			//printk( "%s() - %d: Nand_Select = %d, gEraseCycle[Nand_Select] = %d, EraseCycle = %d\n", __PRETTY_FUNCTION__, __LINE__, Nand_Select, gEraseCycle[Nand_Select], EraseCycle );
			EraseCycle = gEraseCycle[Nand_Select];
		}
		else
		{
 			//printk( "%s() - %d: Nand_Select = %d, gPhysical_NandInfo->wAddrCycle = %d, EraseCycle = %d\n", __PRETTY_FUNCTION__, __LINE__, Nand_Select, gPhysical_NandInfo->wAddrCycle, EraseCycle );
...
void DrvNand_WP_Initial(void)
{
//	nand_func[0] = gp_board_get_config("nand",gp_board_nand_t);
//	nand_func[1] = gp_board_get_config("nand1",gp_board_nand_t);
//	return;
...
{
	//printk( "%s()-%d: nand_func[%d]=%p", __PRETTY_FUNCTION__, __LINE__, Nand_Select, nand_func[Nand_Select] );
	DrvNand_WP_Initial();
  if(nand_func[Nand_Select]!=NULL)
	{
		//printk( "%s()-%d: wp called!\n", __PRETTY_FUNCTION__, __LINE__ );
