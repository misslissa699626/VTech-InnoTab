...
// 8 bits/512 4k page User Data=2 byte, Parity=14 byte, Pseudo=16 byte
static UINT8 all_ff_parity_of_bch_8[] = {
	0xff, 0xff, 0x30, 0x9a, 0x0c, 0x9d, 0x90, 0x48, 0x5f, 0x6e, 0xb6, 0x9a, 0xa7, 0x26, 0xf0, 0x10,
	0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff
};

// 4 bits/512 4k page User Data=9 byte, Parity=7 byte, Pseudo=16 byte
...
SINT32 bch_init_intr(void) 
{
	...
	R_BCH_S332_INT_STATUS = 0x01; /* clear bch interrupt if any not clear by previous code */
	return request_irq(IRQ_BCH, bch_interrupt_isr, IRQF_DISABLED, BCH_DEVICE_NAME, NULL);
}
#endif
...
{
	unsigned long t, ms_timeout, k, j;
  ...
  UINT32 user_data_mode = 0;
	...
	L_RETRY:
		...
		#if defined(BCH_DECODE_DEBUG_1) // william debug
		k = 0;
    ...
    if(iboot_bch_reg==1)
    {
        user_data_mode = 0x50000000; // william:modify to BCH1K60_BITS_MODE, since header is using 60 bits ECC
    }
    ...
    (*(volatile unsigned *)(BCH_S332_BASE+0x14))  =	0x1 ;							//INT_MASK, william must 1
  	...
  	if (ms_timeout == 0) {
  	  ...
  	  for (k=1; k < 10; k++) {
  		...
  		if (retry < 1) {
  			retry++;
  			printk("bch_reset, pageAddr[%u] try again[%lu]\n", pageAddr, retry);
  			goto L_RETRY;
  		}
  		#endif
          	...
          	if ((j >= (data_length - 2)) && (j <= (data_length - 1))) {
            	...
            	BCH_DEBUG_INFO("Warning, page[%u] decode fail detected:not all ff.max_bit_err_cnt[%x]0x18[%x]0x1C[%x]0x20[%x]0xC[%x]0x24[%x]0x0[%x]\n", \
            										...
            										max_bit_err_cnt, \
  	    												...
  	    												(*(volatile unsigned *)(BCH_S332_BASE+0x24)), \
  	    												(*(volatile unsigned *)(BCH_S332_BASE+0x0)) );
  	    			//dump_buffer((unsigned char *)DataSrcAddr, data_length);//comment by Kevin
  	    			//dump_buffer((unsigned char *)ParitySrcAddr, 1024);//comment by Kevin
  	    		}
        }
