...
static DECLARE_MUTEX(dac_sem);
static int Vrefon_Cnt=0;
...
static void audio_clock_enable(int enable)
{
	if( enable ){
		...
		gpHalScuClkEnable(SCU_A_PERI_I2S | SCU_A_PERI_I2SRX | SCU_A_PERI_SAACC, SCU_A, 1);
	}
	else{
		gpHalScuClkEnable(SCU_A_PERI_I2S | SCU_A_PERI_I2SRX | SCU_A_PERI_SAACC, SCU_A, 0);
...
static void sar_vref_ctrl(int isVrefOn)
{
	...
	if(isVrefOn)
	{
		...
		SAACC_PWCTRL = reg | SAR_ENVREF;	// | SAR_VREFSM;// |  SAR_ENZCD;
...
/*		#else
	...
	//	udelay(1000);			//delay 10us
		...
		reg = SAACC_PWCTRL;
		SAACC_PWCTRL = reg | SAR_ENMIC | SAR_ENMICBIAS | 0x1f<<6;
		reg = SAACC_ADCCTRL;									//Add by Simon
		SAACC_ADCCTRL= reg|SAR_ENADL|SAR_VOLCTL;					//Add by Simon
	
		SAACC_LINOUT = 0xFFFF;		//??Only for GPL32900, Add by Simon
		Vrefon_Cnt++;
	}
	else
	{
		...
		else if(Vrefon_Cnt==1)
		{
			audio2_HDPHN_volset(0x1f, 0x1f);			//mute
			audio2_WAVE_ctrl(0);						//DAC OFF
			sar_vref_ctrl(0);
		...
		/*	#else
			gpHalScuClkEnable(SCU_A_PERI_APBDMA_A, SCU_A, 0);
			gpHalScuClkEnable(SCU_A_PERI_I2S, SCU_A, 0);
			gpHalScuClkEnable(SCU_A_PERI_I2SRX, SCU_A, 0);
			gpHalScuClkEnable(SCU_A_PERI_SAACC, SCU_A, 0);
			#endif*/
		}
		Vrefon_Cnt--;
	}		
...
EXPORT_SYMBOL(audio2_Power_ctrl);
...
EXPORT_SYMBOL(audio2_Adins_get);
...
EXPORT_SYMBOL(audio2_HPINS_get);
...
EXPORT_SYMBOL(audio2_HPINS_set);
...
EXPORT_SYMBOL(audio2_WAVE_ctrl);
...
EXPORT_SYMBOL(audio2_WAVE_volset);
...
EXPORT_SYMBOL(audio2_WAVE_volget);
...
{
	long l_vol, r_vol;
	audio2_WAVE_volget(&l_vol, &r_vol);
...
EXPORT_SYMBOL(audio2_WAVE_ismute);
...
EXPORT_SYMBOL(audio2_WAVE_muteget);
...
EXPORT_SYMBOL(audio2_WAVE_muteset);
...
EXPORT_SYMBOL(audio2_LINEOUT_volset);
...
EXPORT_SYMBOL(audio2_LINEOUT_volget);
...
EXPORT_SYMBOL(audio2_LINEOUT_muteget);
...
EXPORT_SYMBOL(audio2_LINEOUT_muteset);
...
EXPORT_SYMBOL(audio2_LINEIN_ctrl);
...
EXPORT_SYMBOL(audio2_LINEIN_volset);
...
EXPORT_SYMBOL(audio2_LINEIN_volget);
...
EXPORT_SYMBOL(audio2_MIC_volset);
...
EXPORT_SYMBOL(audio2_MIC_volget);
...
EXPORT_SYMBOL(audio2_HDPHN_volset);
...
EXPORT_SYMBOL(audio2_HDPHN_volget);
