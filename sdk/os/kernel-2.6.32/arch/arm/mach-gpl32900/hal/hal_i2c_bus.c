...
)
{
	...
	clock = clk_get(NULL, "clk_sys_apb"); 
    clockSysApb = clk_get_rate(clock);
	//printk("clk_sys = %d\r\n", clockSysApb);
	return clockSysApb/1000;
}
 ...
 * @param   clkRate[in]: i2c bus clock rate(KHZ)
 */
static void
...
)
{
	...
	//printk("[%s][%d] run, clkRate = %d,clockSrc =%d\n", __FUNCTION__, __LINE__, clkRate, clockSrc);

	//tmp = SYS_AHB_CLK /(clkRate * 2);
	/** @todo , check clock_sys been change or not, if change flag set , re-get sys_clock */
	// if (clock_src_change(sys_clock))
	//clockSrc = gpHalI2cGetClkRate();

	tmp = clockSrc / 2 / clkRate;

	if(tmp > 0xfff) { /*pre-scale 12bits, so limit to 0xfff*/
		tmp = 0xfff;
	} else if(tmp > 0) {
		tmp --;
	}
...
#else
	ret = HAL_BUSY_WAITING(i2cBusReg->iccr & ICCR_INTPREND, 30);
#endif
...
)
{
	...
	if (cmd == I2C_BUS_READ) {
		...
		i2cBusReg->idsr = aSlaveAddr & 0xfe;
	}	
