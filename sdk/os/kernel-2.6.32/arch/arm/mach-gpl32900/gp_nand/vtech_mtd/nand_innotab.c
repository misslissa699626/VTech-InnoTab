...
static SINT32 nandcard_detect_init(void)
{
	UINT32 pin_index;
	//#define MK_GPIO_INDEX(ch, func, gid, pin)
	pin_index = MK_GPIO_INDEX(0, 0, 24, 7);//Nand card detect pin IOA7
	
	if (gpHalGpioSetPadGrp(pin_index) != 0) {
		ERR_PRINT("gpHalGpioSetPadGrp fail!!");
		return -1;
	}
	
	if (gpHalGpioSetFunction(pin_index, GPIO_FUNC_GPIO) != 0)
	{
		ERR_PRINT("gpHalGpioSetFunction fail!!");
		return -1;
	}
	
	if (gpHalGpioSetDirection(pin_index, GPIO_DIR_INPUT) != 0)
	{
		ERR_PRINT("gpHalGpioSetDirection fail!!");
		return -1;
	}
	
	if (gpHalGpioSetPullFunction(pin_index, GPIO_PULL_HIGH) != 0)
	{
		ERR_PRINT("gpHalGpioSetPullFunction fail!!");
		return -1;
	}
...
static SINT32 is_nandcard_inserted(void)
{
	UINT32 value, pin_index;
	
	//#define MK_GPIO_INDEX(ch, func, gid, pin)
	pin_index = MK_GPIO_INDEX(0, 0, 24, 7);//Nand card detect pin IOA7

	if (gpHalGpioGetValue(pin_index, &value) != 0)
	{
		ERR_PRINT("gpHalGpioGetValue fail!!");
		return 0;
	}
...
//		up(&innotab_nand_sem);
	...
	if(dev_no == 1)
	{
		nandcard_detect_init();
		if(!is_nandcard_inserted())
		{
			ERR_PRINT("nand card not inserted!!\n");
			nand_sema_unlock();
			return (-1);
		}
	}
...
//		up(&innotab_nand_sem);
	...
	if((dev_no == 1) && (!is_nandcard_inserted()))
	{
		ERR_PRINT("nand card not inserted!!\n");
		nand_sema_unlock();
		return -EINVAL;
