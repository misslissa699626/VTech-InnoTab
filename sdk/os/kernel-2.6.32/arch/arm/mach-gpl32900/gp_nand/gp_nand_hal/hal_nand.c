...
void nandSetDmaTiming(struct nand_dma_timing_rec *timing, bool doWrite)
{
	...
	//rFM_AC_TIMING = 0x1f2f3502; //huck modify	
	//rFM_AC_TIMING = 0x1F2A3412;//modified by Kevin, It's said that the hold time will become 24.8ns.
#endif //if 0

	rFM_AC_TIMING = 0x1F2A3534;//modified by Kevin, From nand OTP, Huck's advise
...
static UINT8 nf_handle_cnt[2] = {0, 0}; 

SINT32 Nand_Init(void)   // Nand normal initial, only init very basic issue
{             
	SINT32 ret = 0;

//	printk("%s(): nf_handle_cnt[0]:%d, nf_handle_cnt[1]:%d, Nand_Select:%d\n", __FUNCTION__, nf_handle_cnt[0], nf_handle_cnt[1], Nand_Select);

	if (nf_handle_cnt[Nand_Select] == 0)  // first nand driver initial
	{
...
//	printk("%s(): nf_handle_cnt[0]:%d, nf_handle_cnt[1]:%d, Nand_Select:%d\n", __FUNCTION__, nf_handle_cnt[0], nf_handle_cnt[1], Nand_Select);
	...
	if(nf_handle_cnt[Nand_Select] > 0)
	{
		nf_handle_cnt[Nand_Select]--;
	}

	if(nf_handle_cnt[0] == 0 && nf_handle_cnt[1] == 0)
	{
		printk("**** Suppose to disable Nand/BCH clock here!!\n");
...
SINT16 good_block_check(UINT16 blk_id, UINT32 WorkBuffer)
{
   // ISP 在遇到壞塊後, 無條件將整個 Block 打成0, 如此有助於壞塊檢測
    ...
    SINT16 ret;
		UINT32 ret1, ret2, ret3, ret4;//modified by Kevin
		SINT16 blkSts1;
		SINT16 blkSts2;
		SINT16 blkSts3;
		SINT16 blkSts4;
...
L_TRY_2:
    ret2 = Nand_ReadPhyPage((detect_page_id+tail_offset), WorkBuffer);
...
L_TRY_3:
    ret3=Nand_ReadPhyPage((detect_page_id+tail_offset2), WorkBuffer);
...
L_TRY_4:
    ret4=Nand_ReadPhyPage((detect_page_id2), WorkBuffer);
...
L_TRY_END:
		if((ret1 == 0xFFFFFFFF) && (ret2 == 0xFFFFFFFF) && (ret3 == 0xFFFFFFFF) && (ret4 == 0xFFFFFFFF))
		{
			return 0x5555;//All read page operations fail! return 0x5555 to abort! --Kevin
		}
		else
...
//	UINT32 i;
	...
	if(ret!=0)	// zurong add
	{
		...
		return 0xFFFFFFFF; //added by Kevin
	}

	if(nand_bch_get() == BCH_OFF) //Added by Kevin
	{
		return 0;
	}
