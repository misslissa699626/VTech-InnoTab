...
#ifdef CONFIG_PM
static int gp_sdio_suspend(struct platform_device *pdev, pm_message_t state);
static int gp_sdio_resume(struct platform_device *pdev);
#endif
/**************************************************************************
 *                         G L O B A L    D A T A                         *
 **************************************************************************/
...
static gp_sdio_info_t *gp_sdio_info = NULL;
static unsigned long sys_apb_clk = 0;
...
#ifdef CONFIG_PM	
	.suspend       = gp_sdio_suspend,
	.resume        = gp_sdio_resume,
#endif	
	.driver        = {
		.name  = DRIVER_NAME,
		.owner = THIS_MODULE,
...
static int gp_sdio_send_command(gp_sdio_host_t *host, struct mmc_command *cmd, struct mmc_data *data)
{
    ...
    /*get message from sd detect*/
    if (sd_data->ocr != 0) {
        if(sdCmd.cmd == 0) {
            ret = 0;
            DIAG_INFO("\tgp_sdio_send_command send CMD0.");
            goto out_send_cmd1;
        }
        else if(sdCmd.cmd == 8) {
            ret = -1;
            DIAG_INFO("\tgp_sdio_send_command send CMD8.");
            goto out_send_cmd1;
        }
        else if(sdCmd.cmd == 5 && cmd->arg == 0) {
            memcpy(&cmd->resp[0], &sd_data->ocr, 4);
            sd_data->ocr = 0;
            ret = 0;
            DIAG_INFO("\tgp_sdio_send_command send CMD5 response=0x%8x.", cmd->resp[0]);
            goto out_send_cmd1;
        }
    }
    ...
    if (data->blksz % 16 != 0) {
        ...
        for (iter = 0; iter < sg_map_len; iter++) {
            ...
            if(!ret) {
                ...
                DIAG_ERROR("cmd=%d, arg=0x%x, status=0x%x, blkcnt=%d,blksz=%d, rw=%d.\n",
                    cmd->opcode, cmd->arg, gpHalSDGetStatus(sd_data->device_id), data->blocks, data->blksz, sdCmd.dir);
                ret = -1;
                goto out_send_cmd;
            }
        }
        ...
        for (iter = 0; iter < sg_map_len; iter++) {
            phy_addr = (unsigned char*)sg_phys(&data->sg[iter]);
            phy_left = sg_dma_len(&data->sg[iter]);

            while(phy_left > 0) {
                buf_len = phy_left > data->blksz ? data->blksz : phy_left;
                
                dma_param.buf0 = phy_addr;
                dma_param.ln0 = buf_len;
    		...
    		/* ----- Wait dma finish ----- */
                ret = gp_apbdma0_wait(dma_handle, ((dma_param.ln0>>9) + 2)*100);
                if(ret != 0) {
               ...
               /*if (sdCmd.dir == SDC_WRITE) */{
                   ret = WaitSDStatus_Data(sd_data->device_id, MASK_S_DataComplete);
                   if(ret != SP_TRUE) {
                        DIAG_ERROR("DMA data complete wait time out!\ncmd=%d, arg=0x%x, status=0x%x, blkcnt=%d,blksz=%d.\n",
                            cmd->opcode, cmd->arg, gpHalSDGetStatus(sd_data->device_id), data->blocks, data->blksz);
                        ret = -1;
                        goto out_send_cmd;
                    }
                } 
                gpHalSDClearStatus(sd_data->device_id, MASK_S_DataComplete);
            }
           ret = gpHalSDWaitIdle(sd_data->device_id);
           if(ret != SP_TRUE) {
                DIAG_ERROR("XXXDMA data complete wait time out!\ncmd=%d, arg=0x%x, status=0x%x, blkcnt=%d,blksz=%d.\n",
                    cmd->opcode, cmd->arg, gpHalSDGetStatus(sd_data->device_id), data->blocks, data->blksz);
                ret = -1;
                goto out_send_cmd;
            }
            gpHalSDClearStatus(sd_data->device_id, MASK_S_ClrAllBits);
...
static void gp_sdio_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
    ...
    case MMC_POWER_ON:
        ...
        /*not need break*/
    case MMC_POWER_UP:
        if(ios->clock > 1*1000*1000) {
            ios->clock = 25*1000*1000;
        }/**/
...
irq_out:
   	gpHalSDClearStatus(sd_data->device_id, MASK_S_SdioInterrupt);
	return IRQ_HANDLED;
}
...
static int __devinit gp_sdio_probe(struct platform_device *pdev)
{
    ...
    if(pclk) {
    	sys_apb_clk = clk_get_rate(pclk)/SD_CLK_UNIT;
    }
    else {
    	sys_apb_clk = 270;
    }    
    ...
    sdio_host->platdata->clk_rate = sys_apb_clk * SD_CLK_UNIT;
 ...
 */
static int __devinit gp_sdio_remove(struct platform_device *pdev)
{
...
#else
#define gp_sdio_suspend 	NULL
#define gp_sdio_resume 	NULL
#endif

void gp_sdio_remove_device(void)
{
    DIAG_INFO("\ngp_sdio_remove_device");
    platform_device_unregister(&gp_sdio_device);
    
    return ;
}
EXPORT_SYMBOL(gp_sdio_remove_device);

void gp_sdio_insert_device(
	UINT32 device_id,
	UINT32 ocr)
{
    int ret;
    
    gp_sdio_platdata.device_id = device_id;
    gp_sdio_platdata.ocr = ocr;
    
    DIAG_INFO("\ngp_sdio_insert_device, device_id=%d", device_id);
    ret = platform_device_register(&gp_sdio_device);
    
    return ;
}
EXPORT_SYMBOL(gp_sdio_insert_device);

/**
 * @brief   SDIO driver init
 */
/*static int __init gp_sdio_init(void)*/
int gp_sdio_init(void)
{
    /*int iter;
    int ret;
