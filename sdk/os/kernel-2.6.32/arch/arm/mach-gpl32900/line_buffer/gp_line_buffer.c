...
/**
 * @brief   Linebuffer clock enable/disable
 * @param   enable [in] 0:disable, 1:enable
 * @return  None
 * @see
 */
static void line_buffer_clock_enable(int enable)
{
	gpHalScuClkEnable(SCU_A_PERI_LINEBUFFER, SCU_A, enable);
}
...
/**                                                                         
 * @brief   line_buffer device release                                              
 */                                                                         
static void gp_line_buffer_device_release(struct device *dev)                       
{                                                                           
	DIAG_INFO("remove line_buffer device ok\n");                                      
}                                                                           
                                                                            
static struct platform_device gp_line_buffer_device = {                             
	.name	= "gp-line_buffer",                                                         
	.id	= 0,                                                                  
	.dev	= {                                                                 
		.release = gp_line_buffer_device_release,                                       
	},                                                                        
};   
                                                                      
static void gp_line_buffer_suspend_set( void ){
	
	line_buffer_clock_enable(0);
}
EXPORT_SYMBOL(gp_line_buffer_suspend_set);

static void gp_line_buffer_resume_set( void ){
	
	line_buffer_clock_enable(1);
}
EXPORT_SYMBOL(gp_line_buffer_resume_set);                                                               
                                                                            
#ifdef CONFIG_PM                                                            
static int gp_line_buffer_suspend(struct platform_device *pdev, pm_message_t state){
	gp_line_buffer_suspend_set();                                              
	return 0;                                                                 
}                                                                           
                                                                            
static int gp_line_buffer_resume(struct platform_device *pdev){  
	gp_line_buffer_resume_set();                                                      
	return 0;                                                                 
}                                                                           
#else                                                                       
#define gp_line_buffer_suspend NULL                                                 
#define gp_line_buffer_resume NULL                                                  
#endif                                                                      
                                                                            
/**                                                                         
 * @brief   wdt driver define                                               
 */                                                                         
static struct platform_driver gp_line_buffer_driver = {                                                                             
	.suspend = gp_line_buffer_suspend,                                                
	.resume = gp_line_buffer_resume,                                                  
	.driver	= {                                                               
		.owner	= THIS_MODULE,                                                  
		.name	= "gp-line_buffer"                                                        
	}                                                                     
...
static int __init line_buffer_init(void)
{
	...
	platform_device_register(&gp_line_buffer_device);
	return platform_driver_register(&gp_line_buffer_driver);
...
{
	platform_device_unregister(&gp_line_buffer_device);
	platform_driver_unregister(&gp_line_buffer_driver);
	kfree(line_buffer);
	line_buffer = NULL;
}
