...
/**
* @brief 	Apbdma0 clock setting function.
* @param 	en[in]: Clock enable or disable.
* @return	None.
*/
static void gpHalApbdmaClk(UINT32 en)
{
	scucReg_t* scuc = (scucReg_t*)LOGI_ADDR_SCU_C_REG;
	
	if(en==APBDMA_CLK_EN)
	{
		scuc->scucPeriClkEn |= 0x80;
	}
	else
	{
		scuc->scucPeriClkEn &= ~0x80;	
...
* @return	None.
*/
static void gpHalApbdmaHWRst(void)
{
	scucReg_t* scuc = (scucReg_t*)LOGI_ADDR_SCU_C_REG;
...
void gpHalApbdmaInit(void)
{
	...
	gpHalApbdmaClk(APBDMA_CLK_EN);	
	/* ----- Reset SDMA channel ----- */
	gpHalApbdmaHWRst();	
}
EXPORT_SYMBOL(gpHalApbdmaInit);
...
void gpHalApbdmaUninit(void)
{
	...
	gpHalApbdmaClk(APBDMA_CLK_DIS);	
}
EXPORT_SYMBOL(gpHalApbdmaUninit);
