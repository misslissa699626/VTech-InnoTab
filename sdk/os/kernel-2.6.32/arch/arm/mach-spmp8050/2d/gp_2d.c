...
int gp_2d_draw_bitmap(g2d_draw_ctx_t *draw_ctx)
{
    ...
    if ((draw_ctx->src.type == SP_BITMAP_RGB888 || draw_ctx->src.type == SP_BITMAP_ARGB8888
        || draw_ctx->src.type == SP_BITMAP_BGR888 || draw_ctx->src.type == SP_BITMAP_ABGR8888)
        && (draw_ctx->dst.type == SP_BITMAP_RGB565 || draw_ctx->dst.type == SP_BITMAP_RGAB5515
        || draw_ctx->dst.type == SP_BITMAP_ARGB1555 || draw_ctx->dst.type == SP_BITMAP_ARGB4444
        || draw_ctx->dst.type == SP_BITMAP_BGR565 || draw_ctx->dst.type == SP_BITMAP_BGAR5515
        || draw_ctx->dst.type == SP_BITMAP_ABGR1555 || draw_ctx->dst.type == SP_BITMAP_ABGR4444)) {
        gpHal2dEnableDither(SP_TRUE);
    }
    else {
        gpHal2dEnableDither(SP_FALSE);
    }
...
static int gp_2d_probe(struct platform_device *pdev)
{
	...
	if (ret < 0) {
		DIAG_ERROR("g2d can't get irq %i, err %d\n", IRQ_2D_ENGINE, ret);
		return ret;
	}
...
static int __init gp_2d_init(void)
{
	...
	if (ret) {
		DIAG_ERROR("unable to register device: %d\n", ret);
	}
