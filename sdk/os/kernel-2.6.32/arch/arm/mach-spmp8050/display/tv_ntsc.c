...
{
	int32_t clock_handle;

	printk("[%s:%d]\n", __FUNCTION__, __LINE__);

	gpHalDispSetDevType(HAL_DISP_DEV_TV);

	// request clock manager
	clock_handle = gp_clock_mgr_request(CLOCK_TVOUT);

	/* Enable lcd clock */
	gpHalLcdScuEnable(gPanelInfo.workFreq); // must enable before lcd clk on		
	// release clock manager
	gp_clock_mgr_release(clock_handle);

	gpHalLcdClkEnable(NULL,1);
	scu_tvout_vdec_enable();
