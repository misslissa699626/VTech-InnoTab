    ...
    if (data->blksz % 16 != 0) {
        ...
        for (iter = 0; iter < sg_map_len; iter++) {
            ...
            if(!ret) {
                ...
                DIAG_ERROR("cmd=%d arg=0x%x, status=0x%x, with_data=0x%x, resp type=%d.\n",
                    sdCmd.cmd, cmd->arg, gpHalSDGetStatus(sd_data->device_id), sdCmd.with_data, sdCmd.resp_type);
                ret = -1;
                goto out_send_cmd;
            }
        }
