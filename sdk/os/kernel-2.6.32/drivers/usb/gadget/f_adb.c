				...
				size_t count, loff_t *pos)
{
	...
	//printk("[%s][%d]dev->online [%d]\n",	__FUNCTION__, __LINE__, dev->online);
...
requeue_req:
	...
	if (ret < 0) {
		...
		printk("[%s][%d]adb_read: xfer error, [%x]\n",	__FUNCTION__, __LINE__, ret);
		r = -EIO;
		dev->error = 1;
		goto done;
	} else {
	...
	while (count > 0) {
		...
		if (req != 0) {
			...
			if (ret < 0) {
				...
				printk("[%s][%d]adb_write: xfer error, [%x]\n",	__FUNCTION__, __LINE__, ret);
				dev->error = 1;
				r = -EIO;
				break;
			}
...
adb_function_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	printk("[%s][%d]bInterfaceNumber[%d]\n",	__FUNCTION__, __LINE__, adb_interface_desc.bInterfaceNumber);
		...
		unsigned intf, unsigned alt)
{
	...
	printk("[%s][%d]adb_function_set_alt intf: %d alt: %d\n",	__FUNCTION__, __LINE__, intf, alt);
