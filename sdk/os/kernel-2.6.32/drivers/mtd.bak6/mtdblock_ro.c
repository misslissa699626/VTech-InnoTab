...
int end_mark_found( struct innotab_blockro *blockro )
{
	struct mtd_info *mtd = blockro->dev.mtd;
...
int mtdblock_open(struct mtd_blktrans_dev *dev)
{
	...
	struct innotab_blockro_map *map = &blockro->map;
	int ret = 0, must_have_end = 0, bbt_exist = 0, fast_scan_header_found = -1;
	...
	if ( !blockro->open_cnt )			// Rebuild the logical to physical block mapping the first time this device is opened.
	{
		...
		fast_scan_header_found = header_for_fast_scan_exist( blockro, &must_have_end, &bbt_exist );
		if ( fast_scan_header_found == 1 )
		{
			if ( must_have_end && ! end_mark_found( blockro ) )			// Try block-by-block bad block scanning procedure
			{
				ret = -1;
