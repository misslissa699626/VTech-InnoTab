...
{
	return innotab_nand_erase_block( CHIP_NO(chip), page );
}

static int innotab_mark_bad( struct nand_chip *chip, int page )
{
	return innotab_nand_mark_bd_oob( CHIP_NO(chip), page );
}
...
struct innotab_nand_chip_config gIntNandChip = {
	  ...
	  .part = { { .name		= "Inno_I0_DUMMY",		// For kernel.bin and boot loader, etc.
	  			.offset		= 0,
				.size		= PART_SIZE_0_0	  },
				...
				{ .name		= "Inno_I4_VFAT",		// For game safe, etc.
