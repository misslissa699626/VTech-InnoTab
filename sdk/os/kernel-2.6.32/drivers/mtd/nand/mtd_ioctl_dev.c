...
int vmtd_fops_ioctl(struct inode *inode, struct file *filp, unsigned int cmd, unsigned long cmd_arg)
{
	...
	case VMTD_IOC_MARK_BAD:
		...
		break;
	case VMTD_IOC_MARK_IF_DLG_2011_READY:
		mtdret = innotab_nand_chip_check_if_dlg_2011_ready( phyChipNo, 1 );
		printk( "%s-%s()-%d: VMTD_IOC_CHECK_IF_DLG_2011_READY on chip %d result: %d\n", __FILE__, __FUNCTION__, __LINE__, phyChipNo, mtdret );
		if ( mtdret )
		{
			mtdret = innotab_nand_chip_mark_if_dlg_2011_ready( phyChipNo, 1 );
			printk( "%s-%s()-%d: VMTD_IOC_MARK_IF_DLG_2011_READY on chip %d result: %d\n", __FILE__, __FUNCTION__, __LINE__, phyChipNo, mtdret );
		}
		break;		
	case VMTD_IOC_CHECK_IF_DLG_2011_READY:
		mtdret = innotab_nand_chip_check_if_dlg_2011_ready( phyChipNo, 1 );
		printk( "%s-%s()-%d: VMTD_IOC_CHECK_IF_DLG_2011_READY on chip %d result: %d\n", __FILE__, __FUNCTION__, __LINE__, phyChipNo, mtdret );
		break;		
	default:
